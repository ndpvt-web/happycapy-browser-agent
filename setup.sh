...
if "$VENV/bin/python3" -m playwright install-deps chromium 2>/dev/null; then
    echo "  OK"
else
...
"$VENV/bin/python3" -c "import websockify" 2>/dev/null && echo "  OK: websockify" && CHECKS=$((CHECKS+1)) || \
    echo "  FAIL: websockify not installed. Fix: $VENV/bin/pip install websockify"
...
if [ $CHECKS -ge $TOTAL ]; then
    ...
    echo "Start with:"
    echo "  cd $DEST"
    echo "  $VENV/bin/python3 agent_server.py"
    echo ""
    echo "Or use the startup script:"
    echo "  $DEST/start.sh"
    echo ""
    echo "Environment variables (optional):"
    echo "  DISPLAY_NUM=98       # Xvfb display number (default: 98)"
    echo "  SCREEN_WIDTH=1280    # Virtual display width (default: 1280)"
    echo "  SCREEN_HEIGHT=1024   # Virtual display height (default: 1024)"
    echo "  AGENT_PORT=8888      # Server port (default: 8888)"
else
